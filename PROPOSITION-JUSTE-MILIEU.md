...
**B32 — remarque 5 (13:42) : plus un mot sur « deux calculs ».** Christophe : « je ne sais pas d'où tu sors cette histoire de deux techniques ou de deux calculs indépendants, ce n'est pas ce qu'on veut ; même s'il y a une technique c'est bien, on n'a pas besoin d'en parler ». Retiré de tout le texte utilisateur (Aujourd'hui, écran 5 et sa figure, Comparer, « comment c'est calculé », fiches). On dit : un moment fort, sa date, son sujet, sa rareté. La règle de silence (K = 2) reste une cuisine interne, jamais un argument à l'écran — à trancher avec Christophe si le seuil lui-même doit changer (il conditionne les 3,6 % et le classement de rareté). Bug corrigé dans l'envoi automatique des remarques : la page se republiait avec le cadre de Claude en double et le thème sombre figé (`data-theme="dark"` sérialisé) ; elle ne publie plus que son propre contenu.

**B32 bis — les remarques traitées disparaissent.** Demande de Christophe : pas d'historique inutile. La liste n'affiche que les remarques en attente ; à chaque republication après traitement, le bloc embarqué est vidé des remarques répondues. La réponse de Claude lui arrive par la conversation (voix + texte). Historique complet conservé ici, brouillon par brouillon.

**B33 — remarques 6 et 7 (13:44, 13:46) : Aujourd'hui en clair.** « Ce n'est pas du français, l'utilisateur ne comprendra rien » sur la phrase d'ouverture, et « je ne comprends pas l'information qu'on donne » sur la ligne du 18 septembre. Réécrit : « Aujourd'hui, rien d'important pour toi. On te préviendra quand il y aura quelque chose. La prochaine date qui compte : le 18 septembre. » Horizon en trois lignes qui disent la date, ce qui change, et le geste : « Le 18 septembre, le sujet change — tu quittes ce que tu partages pour tes idées, tes voyages — à régler avant » ; « Jusqu'en 2031, tout est plus contrasté — les hauts plus hauts, les bas plus bas — décider lentement » ; « Décembre 2029, ton prochain moment fort — du 8 au 22, sujet : tes proches, ce qui doit être dit — on te prévient avant ».

**B34 — remarque 8 (13:52) : les cases à cocher deviennent des interrupteurs.** « Un vrai design qui se marie avec le design » pour « Me rappeler le 11 septembre ». Les lignes de rappel et de calendrier sont maintenant des lignes de verre avec le texte à gauche et un interrupteur à droite (piste arrondie, bouton blanc, violet de marque quand c'est activé, bord qui s'allume), comme les réglages d'une app iOS et dans les jetons du dépôt.

**B35 — remarque 9 (13:52) : les mentions « calculé / traduit par IA » retirées.** « Ça sert à rien, c'est du bruit. » Toutes les lignes de provenance (« calculé, puis traduit par IA », « traduit par IA depuis le calcul », « calculé ») disparaissent des écrans et des fiches. La transparence sur l'IA reste une obligation légale (AI Act art. 50) : à porter dans les conditions et l'onboarding de l'app, pas sur chaque écran. Aussi : le badge « 0 » du crayon ne s'affiche plus.

**B35 bis — remarque 10 (13:55).** « À quoi ça sert : quand un fait de ta vie tombe sur une date calculée… » : « je comprends rien ». Phrase retirée ; les dates et les faits parlent d'eux-mêmes.

**B36 — remarques 11, 12, 13 et le point vert (03/09, 14:05).** (11) Les faits de Brad n'ont rien à faire dans l'écran 1 : « on ne peut pas inventer ce qu'un utilisateur a vécu ». Ne restent que les dates calculées de chaque partie (années fortes, bascules, prochain moment fort). (12) Le bandeau « Mercredi 2 septembre 2026 · Brad, 62 ans » retiré. (13) Réglage des notifications dans la fiche finale : trois choix (moments forts seulement, recommandé · + changements de sujet · tout), enregistré dans le navigateur, puis le bloc se replie sur « Réglé : … Tu peux toujours changer ça dans les paramètres ». Le point vert de l'anneau : c'était le bout arrondi du segment Culmination visible avant son tracé (dasharray 100 / offset 100 avec linecap round) ; décalage 101 et bouts droits. Ombre portée retirée.

**B37 → version Christophe (03/09, 14:30).** Anneau : bouts arrondis rétablis, le point vert venait d'un trait de longueur nulle à l'extrémité du 3ᵉ segment (décalage 105 hors du tracé). Halo retiré (« baisse la qualité de lecture »). Boutons d'Aujourd'hui sans « aujourd'hui » en double. Puis, à la demande de Christophe, **une version calculée sur sa propre naissance** (27/09/1977, 00:00, Bruxelles), publiée comme artefact séparé : 154 moments forts sur cent ans, on lui parle 8,1 % du temps (la médiane des 31 vies) ; sujet dominant « toi-même » (69 %), famille de 8 vies sur 31 ; dernier moment fort 21→25 juil. 2026 ; prochain 1ᵉʳ→5 oct. 2026 (création, enfants) ; période en cours 17 juil. 2025 → 10 oct. 2026 (Scorpion, sujet création/enfants), suivante connue dès oct. 2027 (Capricorne, relations) — un trou dans les périodes L2 du moteur entre les deux, à signaler ; années les plus chargées : 2019 (85 j) passée, 2028 (89 j) et 2029 (102 j) à venir ; correspondances : Stephen Hawking 86 (19 moments identiques au même âge), Sanjay Gandhi 78, Oprah Winfrey 78, Demi Moore 66, Conan O'Brien 66. **Absent : l'écran des trois parties de vie** — ce calcul n'est disponible que pour les fiches de la base (AstroLearn par personId), pas pour une naissance libre : route à demander à Marie-Ange. Données : `donnees/christophe-*.json`.


**B38 — un signal suffit pour l'écran du jour (04/09).** Christophe : « je ne veux pas qu'il attende que deux techniques se croisent, ça dilue trop le déclencheur d'insight qui génère de la nouveauté dans l'écran d'accueil ». Mesuré sur Brad (1 819 signaux Zebrapad, naissance 18/12/1963 06:31 Shawnee) : les convergences (K = 2) couvrent 3,5 % des jours ; avec un signal seul, 91 % des jours ont quelque chose en cours (les périodes de fond), 45 % hors périodes de fond, et un **nouveau** signal commence 17 fois en 2026, soit une à trois fois par mois — c'est ça, la nouveauté de l'accueil. Décision appliquée : l'écran du jour dit toujours ce qui est en cours (période « tes idées, tes voyages » depuis le 22 mars, jusqu'en octobre 2027), ce qui arrive (6 sept. : deux semaines travail/santé ; 18 sept. : changement de sujet ; 14–28 oct. : l'argent ; puis 16–30 oct. : ce que tu crées) et le prochain moment fort (déc. 2029). Les convergences restent la définition des **moments forts** : notifications par défaut (3,5 %) et Comparer (la base a été mesurée là-dessus, inchangée). Réglage « Tout ce qui est calculé » renommé « Chaque nouveau signal · une à trois fois par mois ». Message à Marie-Ange réécrit dans ce sens. Données : `donnees/brad-signaux-2026-09-04.json`.
